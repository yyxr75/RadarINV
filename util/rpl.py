...
class RadarSignalProcessing():
    def __init__(self,path_calib_mat,method='PC',device='cpu',lib='CuPy'):
        ...
        ## radar coordinate
        self.azimuth_coord = self.AoA_mat['Azimuth_table']
        self.elevation_coord = self.AoA_mat['Elevation_table']
    ...
    def run(self,adc0,adc1,adc2,adc3):
        ...
        RD_spectrums = RD_spectrums - np.mean(RD_spectrums)
        if(self.method=='RD'):
            return RD_spectrums
        elif(self.method=='PC'):
    ...
    def run_(self, RD_spectrums):
        # 2- Remove DC offset
        # complex_adc = complex_adc - np.mean(complex_adc, axis=(0,1))
        RD_spectrums = RD_spectrums - np.mean(RD_spectrums, axis=(0,1))
        import pdb;pdb.set_trace()
