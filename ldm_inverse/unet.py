...
class UNet(nn.Module):
    ...
    def forward(self, x):
        x1 = self.inc(x) # [64,512,768]
        x2 = self.down1(x1) # [128,256,384]
        x3 = self.down2(x2) # [256,128,192]
        x4 = self.down3(x3) # [512,64,96]
        x5 = self.down4(x4) # [1024,32,48]
        x = self.up1(x5, x4) # [512,64,96]
        x = self.up2(x, x3) # [256,128,192]
        x = self.up3(x, x2) # [128,256,384]
        x = self.up4(x, x1) # [64,512,768]
        # x = self.up5(x)
        # x = self.up6(x)
        # x = self.up7(x) #
